...
class Desk
    ...
    def game_over?(amplua)
        #три по горизонтали
        self.desk.each  do |row|
            if (row.all? {|el| el == amplua})
                puts "Победил игрок #{amplua}. Игра окончена."
                return  true
            end
        end
        ...
        self.cols.times do |column|
            status = true
            self.rows.times do |row|
                if (!(self.desk[row][column] == self.desk[0][column] && self.desk[row][column] == amplua) )
                    status = false
                end
            end
        ...
        self.rows.times do |index|
            if !(self.desk[index][index] == self.desk[0][0] && amplua == self.desk[0][0]) then status = false end
        ...
        status = true
        self.rows.times do |index|
            if (self.desk[index][self.desk.length - 1 - index] != self.desk[0][self.desk.length - 1] || self.desk[0][self.desk.length - 1] != amplua)
                status = false
            end
        end
        if status == true
